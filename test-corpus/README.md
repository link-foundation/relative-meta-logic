...
and columns, or the E006 error it fails with, and the text and logical lines
the prepare step returns. `js/tests/lino-frontend.test.mjs` and
`rust/tests/lino_frontend_tests.rs` check every case.
`experiments/lino-frontend/case-entry.mjs` prints the entry for a new source,
and `experiments/lino-frontend/differential.mjs` reads generated documents with
