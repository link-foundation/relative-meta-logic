...
fn represents_rml_source_losslessly_through_meta_language() {
    ...
    assert_eq!(
        parse_rml_links_via_meta_language(RML_SAMPLE).expect("the sample is valid LiNo"),
        vec![
            "(a: a is a)".to_string(),
...
fn keeps_rml_parser_and_evaluator_results_identical_after_meta_language_round_trip() {
    let report = rml_meta_language_parity_report(RML_SAMPLE).expect("the sample is valid LiNo");
...
#[test]
fn keeps_an_unmatched_parenthesis_inside_a_quote_or_a_comment_through_the_round_trip() {
    let source = "# it's (\n(a \"(\" b)\n";
    let report = rml_meta_language_parity_report(source).expect("the source is valid LiNo");
    ...
    assert_eq!(
        parse_rml_links_via_meta_language(source).expect("the source is valid LiNo"),
        vec!["(a '(' b)".to_string()]
    );
    assert!(report.round_trip_ok);
    assert!(report.link_parity_ok);
...
#[test]
fn rejects_source_that_is_not_lino_with_the_position_of_the_failure() {
    let source = "(a: a is a)\n(? (a = a)";
    let errors = [
        parse_rml_links_via_meta_language(source).unwrap_err(),
        rml_meta_language_parity_report(source).unwrap_err(),
        meta_language_feature_report(source).unwrap_err(),
    ];
    for error in errors {
        assert_eq!(
            error.to_string(),
            "LiNo parse failure: unexpected end of input"
        );
...
fn summarizes_available_meta_language_features_for_issue_181() {
    let report = meta_language_feature_report(RML_SAMPLE).expect("the sample is valid LiNo");
